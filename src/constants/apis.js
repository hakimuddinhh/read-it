export const base = 'https://www.reddit.com';
const baseV1 = 'https://www.reddit.com/api/v1/';
const baseAuthV1 = 'https://oauth.reddit.com/api/v1/';

export const baseAuth =  'https://oauth.reddit.com/'

export const profileURL =  baseAuthV1 + 'me';
export const bestListingsURL =  baseAuth + 'best?raw_json=1';
export const accessTokenURL =  baseV1 + 'access_token';


export const bestListingGuestURL = base + '/best.json?raw_json=1';

